...
(function () {
  ...
  function updateBankHolidayBooker() {
    ...
    if (message) {
      const startLabel = formatHumanDate(effectiveStart);
      const endLabel = formatHumanDate(rangeEnd);
      message.textContent = `Highlighting bank holidays between ${startLabel} and ${endLabel} in this leave accrual year.`;
    }
    ...
    if (lastDefault && typeof lastDefault === 'object') {
      ...
      if (effectiveStart && rangeEnd) {
        ...
        if (startLabel && endLabel) {
          return `Bank holidays automatically calculated${countText} between ${startLabel} and ${endLabel} in the configured leave accrual year based on the selected start date.`;
        }
      }
      if (Number.isFinite(count)) {
        return `Bank holidays automatically calculated${countText} based on the selected start date within the configured leave accrual year.`;
      }
    }
    return 'Bank holidays value automatically calculated using the configured leave accrual year when available.';
  }
  ...
  function getFourDayBookerReportData() {
    ...
    return {
      ...base,
      message: `Highlighting bank holidays between ${startLabel} and ${endLabel} in this leave accrual year.`,
...
<html lang="en">
  ...
  function updateFourDayWeekBankHolidayDefault({ force = false } = {}) {
    ...
    if (bankHolidayHelp) {
      if (!bankHolidayState.events.length) {
        ...
        bankHolidayHelp.textContent = 'Unable to determine the leave accrual year for the selected start date.';
      } else {
        const { count, effectiveStart, rangeEnd } = computed;
        const startLabel = formatHumanDate(effectiveStart);
        const endLabel = formatHumanDate(rangeEnd);
        bankHolidayHelp.textContent = `Counting ${count} bank holidays remaining between ${startLabel} and ${endLabel} in this leave accrual year.`;
      }
    }
  }
  ...
  function refreshLeaveYearCopy() {
    ...
    const startMessage = fullRange
      ? `Used to determine the leave accrual year (${fullRange}) and remaining bank holidays.`
      : 'Used to determine the leave accrual year and remaining bank holidays.';
    $$('[data-leave-year-start-note]').forEach((el) => {
      el.textContent = startMessage;
    });

    const bookerIntroMessage = fullRange
      ? `Pick the regular non-working day to see which bank holidays still fall on it before the end of the leave accrual year (${fullRange}).`
      : 'Pick the regular non-working day to see which bank holidays still fall on it before the end of the leave accrual year.';
    $$('[data-leave-year-booker-intro]').forEach((el) => {
      el.textContent = bookerIntroMessage;
    });

    const bookerHintMessage = fullRange
      ? `We'll use the start date above to work out the rest of the leave accrual year (${fullRange}).`
      : "We'll use the start date above to work out the rest of the leave accrual year.";
    $$('[data-leave-year-booker-hint]').forEach((el) => {
      el.textContent = bookerHintMessage;
    });

    const nineIntroMessage = fullRange
      ? `Choose the first non-working day in your every other week pattern to spot overlapping bank holidays in this leave accrual year (${fullRange}).`
    ...
    const nineHintMessage = fullRange
      ? `We'll use the configured leave accrual year (${fullRange}) to work out the matching range.`
      : "We'll use the configured leave accrual year to work out the matching range.";
    $$('[data-leave-year-nine-booker-hint]').forEach((el) => {
      el.textContent = nineHintMessage;
    });

    const settingsNote = fullRange
      ? `Choose when the leave accrual year begins. We'll apply the selected month and day every year and calculate the end date automatically. Current cycle: ${fullRange}.`
      : "Choose when the leave accrual year begins. We'll apply the selected month and day every year and calculate the end date automatically.";
  ...
  function renderBankHolidays({ updateYears = false } = {}) {
    ...
    if (rangeInfo) {
      ...
      rangeInfo.textContent = label
        ? `Showing leave accrual year: ${label}.`
        : 'Set the leave accrual year in Settings to view matching bank holidays.';
    }

...
(function () {
  ...
  function getNineDayFortnightBookerElements() {
    ...
    const elements = {
      ...
      list: card.querySelector('[data-nine-day-booker-matches]'),
    };
  ...
  function updateNineDayFortnightBookerYearOptions() {
    const booker = getNineDayFortnightBookerElements();
    if (!booker) return;
    const { yearSelect } = booker;
    ...
    const previousSelection = (() => {
      const stored = nineDayFortnightState.selectedYear || yearSelect.value || '';
    ...
    const resolved = years.includes(previousSelection)
      ? previousSelection
    ...
    nineDayFortnightState.selectedYear = yearSelect.value;
  }

  function updateNineDayFortnightBooker() {
    const booker = getNineDayFortnightBookerElements();
    if (!booker) return;
    const { yearSelect, startDate, message, results, list } = booker;
    ...
    const yearValue = yearSelect ? Number.parseInt(yearSelect.value || '', 10) : Number.NaN;
    if (!yearSelect || !yearSelect.value || Number.isNaN(yearValue)) {
      if (message) {
        message.textContent = 'Select a year to highlight every other week matches.';
      }
      return;
    }

    nineDayFortnightState.selectedYear = String(yearValue);
    ...
    if (!matches.length) {
      if (message) {
        message.textContent =
          'No bank holidays align with this every other week pattern within the selected window.';
      }
      return;
    }
    ...
    if (list) {
      matches.forEach((match) => {
        ...
        const title = document.createElement('p');
        title.className = 'font-medium text-gray-900 dark:text-gray-100';
        title.textContent = match.title || 'Bank holiday';
        entry.appendChild(title);
        const dateLine = document.createElement('p');
        dateLine.className = 'text-xs text-gray-600 dark:text-gray-400';
  ...
  function initializeNineDayFortnight() {
    ...
    updateNineDayFortnightBankHolidayDefault({ force: true });
    updateNineDayFortnightSummary();
    updateNineDayFortnightBookerYearOptions();
    updateNineDayFortnightBooker();
  }
